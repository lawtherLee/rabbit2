...
import { UserAddress } from "@/types/checkout.ts";
...
const confirm = async () => {
  if (formData.value.id) {
    await checkoutStore.updateAddress(formData.value);
  } else {
    await checkoutStore.addAddress(formData.value);
  }
...
const editAddress = () => {
  ...
  if (checkoutStore.showUserAddress) {
    // 防止修改的时候，直接影响到当前显示的地址
    formData.value = {
      ...checkoutStore.showUserAddress,
    };
  }
};
...
watch(visible, (value) => {
  if (value) {
    selectAddress.value = checkoutStore.showUserAddress!;
  }
});
  ...
  <div class="checkout-address">
    ...
    <XtxDialog title="切换收货地址" v-model="visible">
      <template #body>
        <div
          class="text item"
          v-for="item in checkoutStore.checkoutInfo.userAddresses"
          :key="item.id"
